...
В репозитории есть папка **`admin-frontend/`** — UI на AdminLTE 3 + PHP + JS (без npm).

- API остаётся в `public_html/` → `/api/v1/*`
- Админка: document root на `admin-frontend/public/` (отдельный поддомен или vhost)
- В `admin-frontend/app/Config/api.php` укажите URL API, например `https://identity.xsdk.ru/api/v1`
...
# Админка
cd admin-frontend/public && php -S localhost:8080 router.php
```

Подробнее: `admin-frontend/README.md`.
